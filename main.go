...
var (
totalAlertsRequests = prometheus.NewCounterVec(
	prometheus.CounterOpts{
		Name: "kubearmor_relay_logs_total",
		Help: "Total number of logs generated from Kubearmor Relay based on PodName, Namespace and Container",
	}, []string{"PodName", "NamespaceName", "ContainerName", "HostName"})
...
func init() {
	prometheus.MustRegister(totalAlertsRequests)
...
func GetPrometheusAlerts(wg *sync.WaitGroup) {
	...
	for {
		...
		fmt.Println(alertIn)
		totalAlertsRequests.WithLabelValues(alertIn.PodName, alertIn.NamespaceName, alertIn.ContainerName, alertIn.HostName).Add(1)
